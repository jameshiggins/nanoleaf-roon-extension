...
/**
 * Panel ownership: the renderer must take the panels only while Roon is playing and
 * hand them back exactly as it found them. Regression cover for the rewrite that
 * streamed black frames forever and never restored the panels' original effect.
 */
...
/** Records every call so tests can assert on order, not just occurrence. */
function fakeClient({ effect = 'Autumn Night Cosy', power = false } = {}) {
  return {
    calls: [],
...
test('acquire saves the current effect and power, powers on, enters extControl', async () => {
  const client = fakeClient({ effect: 'Autumn Night Cosy', power: false });
  const { renderer } = make({ client });
  renderer.start();
  await renderer.acquire();

  assert.equal(renderer.acquired, true);
  assert.equal(renderer.savedEffect, 'Autumn Night Cosy');
...
test('release is debounced, then restores the saved effect', async () => {
  const client = fakeClient({ effect: 'Autumn Night Cosy', power: true });
  ...
  assert.ok(client.calls.includes('selectEffect(Autumn Night Cosy)'), 'restored the effect');
  renderer.stop();
});
...
test('panels that were OFF before are powered back off after restore', async () => {
  ...
  const select = client.calls.indexOf('selectEffect(Autumn Night Cosy)');
...
test('resuming within the debounce window cancels the pending release', async () => {
  ...
  assert.ok(!client.calls.includes('selectEffect(Autumn Night Cosy)'), 'never restored');
  renderer.stop();
});
...
test('stops streaming frames once released', async () => {
  ...
  const afterAcquire = streamer.frames.length;
  assert.ok(afterAcquire > 0, 'frames flow while acquired');
...
test('acquire is idempotent — a second playing event does not re-save *Dynamic*', async () => {
  const client = fakeClient({ effect: 'Autumn Night Cosy', power: false });
  const { renderer } = make({ client });
  renderer.start();
  await renderer.acquire();
  ...
  assert.equal(renderer.savedEffect, 'Autumn Night Cosy', 'saved effect survives re-acquire');
